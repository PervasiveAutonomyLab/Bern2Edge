"""
HIGGS-Small — Bernstein-vs-ReLU summary table, computed from model weights.
...
Checkpoint -> fold mapping: each file stores ``seed`` = 42 + fold_id, and the
folds come from ``higgs_small_dataloaders_kfold(seed=42)`` (the same split
protocol used at training time).
...
DATA_SEED   = 42      # must match the seed used at training time


def arch_label(arch):
    """Render a width list as the paper's set notation, e.g. {28, 16, 2}."""
...
def print_table(summary):
    """Print a console table mirroring the paper's HIGGS-Small block."""
    print("\n" + "=" * 78)
    print(f"{'Arch.':<16}{'Act.':<7}{'Acc. (%)':<16}{'Train CE':<11}"
    ...
    for arch, grp in summary.groupby("arch", sort=False):
        for _, row in grp.iterrows():
            ...
            print(f"{arch:<16}{row.activation:<7}{acc:<16}"
                  f"{row.ce_train_mean:<11.3f}{row.ce_test_mean:<10.3f}{delta:<14}")
        print("-" * 78)


def main():
    folds, _, _ = higgs_small_dataloaders_kfold(n_splits=5, batch_size=512, seed=DATA_SEED)
