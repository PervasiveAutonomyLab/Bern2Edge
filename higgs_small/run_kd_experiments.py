"""
HIGGS-Small — knowledge-distillation training driver (Bern2Edge).
...
``TeacherMLP`` teacher, across the 5 cross-validation folds (seed = 42 + fold),
and saves one checkpoint per (config, fold).

Pipeline per run (mirrors higgs_exp.ipynb):
...
from bern2edge.models import TeacherMLP, FCModel                       # noqa: E402
from bern2edge.data import higgs_small_dataloaders_kfold              # noqa: E402
...
TEACHER_WEIGHTS = "higgs_small_teacher.pth"   # dict checkpoint (model_state_dict)
TEACHER_LAYERS  = [231, 121]
TEACHER_DROPOUT = 0.0
LEARNING_RATE   = 0.006
WEIGHT_DECAY    = 0.0
RANGE_PENALTY   = 1e-2     # Bernstein out-of-bounds penalty weight
BATCH_SIZE      = 512
DATA_SEED       = 42       # fold seed; per-fold student seed = DATA_SEED + fold_id

# ── Experiment table ─────────────────────────────────────────────────────────
# One Bernstein and one ReLU example. To reproduce another architecture, copy a
# bern/relu pair and edit the fields:
#   layer_sizes : full width list, input ... output (input=28, output=2)
#   degree      : Bernstein polynomial degree (3); use None for ReLU
...
RUNS = [
    ...
    ([28, 16, 2],       "bern", 3,      0.5,   2.0),    # Bernstein student
    ([28, 16, 2],       "relu", None,   0.85,  2.0),    # ReLU baseline
    # Published table also includes:
    # ([28, 16, 8, 2],  "bern", 3,      0.5,   2.0),
    # ([28, 16, 8, 2],  "relu", None,   0.85,  2.0),
    # ([28, 128, 2],    "bern", 3,      0.7,   2.0),
    # ([28, 128, 2],    "relu", None,   0.85,  2.0),
]
...
def main():
    parser = argparse.ArgumentParser(description="HIGGS-Small KD training driver.")
    ...
    folds, d_in, n_classes = higgs_small_dataloaders_kfold(
        n_splits=5, batch_size=BATCH_SIZE, seed=DATA_SEED
    )
    teacher = TeacherMLP(d_in=d_in, d_layers=TEACHER_LAYERS,
                         dropout=TEACHER_DROPOUT, d_out=n_classes).to(device)
    ckpt = torch.load(TEACHER_WEIGHTS, map_location=device)
