...
| Table IV — LUT BNN vs rule-network hardware | **Missing** | Only the typeset table in the PDF is present |
...
| Table VI — end-to-end hardware | **Missing as an experiment** | Some BNN values overlap Tables I/III, but W8A8 teacher synthesis and complete provenance are absent |
...
| Table IX — fallback ablation | **Missing as a table** | Fallback implementations exist, but no four-way result CSV, renderer, or hardware measurements |
...
| Figure 9 — penalty curves | **Partial source data:** Table VIII covers selected joint settings, not the dense one-dimensional sweeps shown | No figure renderer |
| Figure 10 — sparsity/BRAM curve | **Missing** | No k-sweep result data, HLS measurements, or plotting script |
...
3. CSV data and deterministic plot scripts for Figures 7–10.
...
## Paper figures in the README

Add **Figure 1 only** near the overview if the authors have the original vector
or high-resolution source. It communicates the teacher → distilled Bernstein
student → LUT/rule deployment flow better than prose. Do not paste screenshots
of all result plots into the README: they make the artifact heavier and can
suggest that a displayed figure is reproducible when its source data and script
are absent. Once Figures 7–10 have CSV inputs and plotting scripts, link their
regenerated outputs from this coverage file.
