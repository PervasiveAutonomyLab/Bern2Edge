...
// ============================================================
// Bernstein FFN sub-block top level (core: fc1 -> bern -> fc2).
//   fc1: 312 -> 600, FUSED weights/bias (output already ~[0,1])
//   act: per-channel degree-15 Bernstein via LUT + lerp
//   fc2: 600 -> 312, wide_t output
// Weights are streamed from DRAM (m_axi); only activations + the per-channel
// LUT are on-chip.
// ============================================================

static data_t input_buf[INPUT_DIM];
static data_t hidden_buf[HIDDEN_DIM];      // fc1 output (~[0,1])
static data_t act_buf[HIDDEN_DIM];         // bernstein output
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    // act_buf read 8-wide in fc2 inner product
#pragma HLS array_partition variable=act_buf cyclic factor=AXI_BLOCK_SIZE
    ...
    bernstein_layer(act_buf, hidden_buf);
