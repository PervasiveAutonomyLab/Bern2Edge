"""
Cover Type — knowledge-distillation training driver (Bern2Edge).
...
``TeacherMLP`` teacher, across ``k`` cross-validation folds (seed = 1000 + fold),
and saves one checkpoint per (config, fold).

Pipeline per run (mirrors ``covertype_experiments.ipynb``):
...
Run:
    python run_kd_experiments.py                 # full reproduction (k=5, 300 ep)
...
TEACHER_WEIGHTS = "covertype_teacher_weights.pth"
TEACHER_LAYERS  = [1024, 1024, 512]
TEACHER_DROPOUT = 0.1
LEARNING_RATE   = 0.006
WEIGHT_DECAY    = 1e-4
RANGE_PENALTY   = 1e-2     # Bernstein out-of-bounds penalty weight
BATCH_SIZE      = 1024
DATA_SEED       = 42       # fixed split seed for covertype_dataloaders_kfold

# ── Experiment table ─────────────────────────────────────────────────────────
# One Bernstein and one ReLU example. To reproduce another architecture, copy a
# bern/relu pair and edit the fields:
#   layer_sizes : full width list, input ... output (input=54, output=7)
...
RUNS = [
    ...
    ([54, 64, 32, 7],      "bern", 3,      0.5,   2.0),   # Bernstein student
    ([54, 64, 32, 7],      "relu", None,   0.5,   2.0),   # ReLU baseline
    # Published table also includes:
    # ([54, 128, 64, 7],   "bern", 5,      0.0,   1.0),
    # ([54, 128, 64, 7],   "relu", None,   0.5,   2.0),
    # ([54, 256, 128, 7],  "bern", 5,      0.5,   2.0),
    # ([54, 256, 128, 7],  "relu", None,   0.5,   2.0),
]
...
def main():
    parser = argparse.ArgumentParser(description="Cover Type KD training driver.")
    ...
    folds, d_in, n_classes = covertype_dataloaders_kfold(
        batch_size=BATCH_SIZE, k=5, seed=DATA_SEED, test_size=0.15
    )
