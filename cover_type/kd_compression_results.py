"""
Cover Type — Bernstein-vs-ReLU summary table, computed from model weights.
...
Checkpoint -> fold mapping: each file stores ``seed`` = 1000 + fold_id, and the
folds come from ``covertype_dataloaders_kfold(seed=42)`` (the training split).
...
def arch_label(arch):
    """Render a width list as the paper's set notation, e.g. {54, 64, 32, 7}."""
...
def print_table(summary):
    """Print a console table mirroring the paper's Cover Type block."""
    print("\n" + "=" * 78)
    print(f"{'Arch.':<20}{'Act.':<7}{'Acc. (%)':<16}{'Train CE':<11}"
    ...
    for arch, grp in summary.groupby("arch", sort=False):
        for _, row in grp.iterrows():
            ...
            print(f"{arch:<20}{row.activation:<7}{acc:<16}"
                  f"{row.ce_train_mean:<11.3f}{row.ce_test_mean:<10.3f}{delta:<14}")
        print("-" * 78)


def main():
    folds, _, _ = covertype_dataloaders_kfold(batch_size=1024, k=5, seed=DATA_SEED, test_size=0.15)
